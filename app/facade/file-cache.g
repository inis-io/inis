...
func init() {
	CacheFile, _ = NewFileCache("runtime/cache", 3600)
}
...
func NewFileCache(dir any, expire any, prefix ...any) (*FileCache, error) {
	...
	client := &FileCache{
		...
		expire: cast.ToDuration(expire) * time.Second,
	}
...
func (this *FileCache) Set(key any, value []byte, expire ...any) (ok bool) {
	...
	if len(expire) > 0 {
		exp = cast.ToDuration(expire[0]) * time.Second
	}
...
func (this *FileCache) SetE(key any, value []byte, expire time.Duration) (err error) {
	...
	this.items[cast.ToString(key)] = &FileCacheItem{time.Now().Add(expire), name}
...
func (this *FileCache) GetInfo(key any) (info map[string]any) {
	...
	if item.expire.IsZero() {
		...
		expire = int64(item.expire.Sub(time.Now()).Seconds())
	}
